...
class RubyText(tk.Text):
    ...
    def insert_segments(self, index, segments: Sequence[RubySegment],
        ...
        with self._editable():
            ...
            try:
                for base, ruby in segments:
                    if ruby:
                        self._insert_pair(at, base, ruby)
                    elif base:
    ...
    def _insert_pair(self, index, base: str, ruby: str) -> None:
        ...
        base_label = tk.Label(frame, text=base, font=self.base_font,
                              fg=self._kanji_fg, bg=self._ruby_bg, bd=0,
                              padx=RUBY_PAD_X, pady=0, anchor='center')
    ...
    def insert_notation(self, index, notation: str, tags=None) -> None:
    ...
    def fit_height(self, available_px: Optional[int] = None,
        ...
        rows = max(1, math.ceil(needed / layout.row_plain))
        try:
